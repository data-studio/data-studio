<?php get_header(); ?>
  ...
  <?php if (have_posts()): while (have_posts()) : the_post(); ?>
    ...
    <?php $back_id = (int) get_field( 'path_web_service_id', get_the_ID() )->ID; ?>
    <div class="back-button-container">
      <a id="BackButton"
        href="<?php the_permalink( $back_id ); ?>"
        title="<?php echo get_field( 'web_service_name', $back_id ); ?>">
        <span class="material-icons">
          chevron_left
        </span>
        <span>
          <?php echo get_field( 'web_service_name', $back_id ); ?>
        </span>
      </a>
    </div>

    <h1><?php echo get_field( 'path_uri', get_the_ID() ); ?></h1>
    ...
    <div id="PathTabbedContent"
      class="tab-contents-wrapper">
      <div class="tab-design">
        <?php get_template_part( 'parts/tabs/path/design-path' ); ?>
      </div>
      <div class="tab-discuss">
        <?php get_template_part( 'parts/tabs/path/discuss-path' ); ?>
      </div>
      <div class="tab-configure">
        <?php get_template_part( 'parts/tabs/path/configure-path' ); ?>
      </div>
      <div class="tab-build">
        <?php get_template_part( 'parts/tabs/path/build-path' ); ?>
      </div>
    </div>
    ...
    (function ($) {"use strict";
      $.dsTabGroup({
        tabs: [
          {
            $toggleEl: $( '#PathNavigation li.tab-design' ),
            $contentEl: $( '#PathTabbedContent div.tab-design' ),
          },
          {
            $toggleEl: $( '#PathNavigation li.tab-discuss' ),
            $contentEl: $( '#PathTabbedContent div.tab-discuss' ),
          },
          {
            $toggleEl: $( '#PathNavigation li.tab-configure' ),
            $contentEl: $( '#PathTabbedContent div.tab-configure' ),
          },
          {
            $toggleEl: $( '#PathNavigation li.tab-build' ),
            $contentEl: $( '#PathTabbedContent div.tab-build' ),
          },
        ],
      })
    })(jQuery);
